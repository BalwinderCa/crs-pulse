...
export default function AnalyticsScreen() {
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <EmptyState
          ...
          description="Analytics will appear once draws are published."
        />
      </SafeAreaView>
    );
  }
  ...
  const trendBadge = data.trend === 'rising'  ? 'success' as const
    : data.trend === 'falling' ? 'danger' as const
    : 'neutral' as const;
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.statsGrid}>
          <StatCard label="Average Cutoff" value={data.average_cutoff} />
          <StatCard label="Highest Cutoff" value={data.highest_cutoff} />
          <StatCard label="Lowest Cutoff"  value={data.lowest_cutoff} />
          <StatCard label="Total Draws"    value={data.total_draws} />
        </View>
        ...
        {chartValues.length > 1 && (
          <Card padding={0} style={styles.chartCard}>
            ...
            <LineChart
              ...
              yAxisSuffix=""
              chartConfig={{
                backgroundGradientFrom: palette.surfaceCard,
                backgroundGradientTo:   palette.surfaceCard,
        ...
        <Card>
          ...
          <Text style={styles.bigStat}>
            {data.total_invitations.toLocaleString()}
          </Text>
...
const styles = StyleSheet.create({
  ...
  periodText: { color: palette.textSecondary, fontSize: typography.sm, fontWeight: typography.medium },
  ...
  statCardLabel: { color: palette.textSecondary, fontSize: typography.xs },
  statCardValue: { color: palette.white, fontSize: typography['2xl'], fontWeight: typography.bold },
  trendCard: { marginHorizontal: spacing.base, gap: spacing.sm },
  trendLabel: { color: palette.textSecondary, fontSize: typography.sm, fontWeight: typography.medium },
  trendRow: { gap: spacing.sm },
  trendDesc: { color: palette.textSecondary, fontSize: typography.sm, lineHeight: 20 },
  chartCard: { marginHorizontal: spacing.base, overflow: 'hidden', padding: spacing.base },
  chartTitle: { color: palette.textSecondary, fontSize: typography.sm, marginBottom: spacing.sm },
  chart: { borderRadius: borderRadius.md },
  bigStat: { color: palette.white, fontSize: typography['3xl'], fontWeight: typography.bold },
  statSub: { color: palette.textMuted, fontSize: typography.xs },
});
